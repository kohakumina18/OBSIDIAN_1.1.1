...
Current Ubuntu vault root is:

```bash
/home/nvakhoa/Documents/BA_Obsidian_Vault
```

The backslashes sometimes shown before underscores are Markdown escaping and are not part of the filesystem path. The Windows vault location, when working on Windows, is:
...
- `PPJxStratova.AI`, `PPJxQSee.AI` (QSee.AI), `PPJxNUNOX.ScanTrial` (NUNOX), `PPJ.UIT.ACADEMIC.COLLABORATION.v1.1` (UIT), `PROD.COWASH.v2.0`, and `QC.Primo1D.RFID.Thread.v1.0` are third-party projects in `EXTERNAL DEVELOPMENT`, not internal PPJ development.
- `PUR.HM.LabelO.Processing.Automation.v1.0` remains in `INTERNAL DEVELOPMENT` with `status: On Hold`.
