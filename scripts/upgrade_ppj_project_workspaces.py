...
GLOBAL_CANVAS = PROJECT_ROOT / "Canvas" / "PPJ_Executive_Board_v2.canvas"
...
def discover_projects(selected_code: str | None = None) -> tuple[list[Project], list[str], list[str]]:
    ...
    preferred_codes = set(preferred_by_note.values())
    # Preferred (canonical) codes claim their note first so legacy aliases of the same note are skipped.
    for code in sorted(evidence, key=lambda c: (c not in preferred_codes, c.casefold())):
    ...
    if selected_code is None:
        ...
        fd = by_code.get("FAB_FabricDatamart_v2.2.0") or by_code.get("FD.Datamart.v2.2")
        cpd = by_code.get("CPD_VisualSampleDatamart_v1.1.0") or by_code.get("CPD.Datamart.v1.1")
        ...
        ai_hub = [project for project in projects if identity_key(project.code) in (identity_key("PPJ.AI.Hub.v2.1"), identity_key("AI_ApplicationHub_v2.1.0"))]
        if len(ai_hub) != 1:
