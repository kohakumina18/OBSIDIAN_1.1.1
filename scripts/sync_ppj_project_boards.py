...
ALIAS_MAP_PATH = VAULT_ROOT / "03_Projects" / "_Registry" / "PPJ_PROJECT_ALIAS_MAP.md"
...
def load_registered_alias_groups() -> list[set[str]]:
    """Load the authoritative canonical/legacy/file/workspace equivalence groups."""
    if not ALIAS_MAP_PATH.exists():
        return []
    groups: list[set[str]] = []
    for raw_line in read_text(ALIAS_MAP_PATH).splitlines():
        if not raw_line.lstrip().startswith("|"):
            continue
        cells = [cell.strip() for cell in raw_line.strip().strip("|").split("|")]
        if len(cells) != 5 or cells[4].casefold() != "registered":
            continue
        canonical, alias_text, current_file, workspace, _ = cells
        values = [canonical, current_file, workspace]
        values.extend(item.strip() for item in alias_text.split(";") if item.strip())
        keys = {identity_key(value) for value in values}
        keys.discard("")
        if keys:
            groups.append(keys)
    return groups
...
def expand_project_aliases(projects) -> None:
    """Add aliases from the authoritative map without changing project metadata."""
    alias_groups = load_registered_alias_groups()
    for project in projects:
        matched = [group for group in alias_groups if project.aliases.intersection(group)]
        for group in matched:
            project.aliases.update(group)
...
def main() -> int:
    ...
    expand_project_aliases(projects)
