...
CANVAS_PATH = ROOT / "03_Projects/Canvas/PPJ_Executive_Board_v2.canvas"
SNAPSHOT_JSON = ROOT / "03_Projects/_Registry/Portfolio_Snapshots/PPJ_PORTFOLIO_SNAPSHOT_20260918.json"
...
def snapshot_markdown(snapshot: dict[str, Any]) -> str:
    ...
    exceptions = snapshot.get("governance_exceptions") or [
    ...
    lines += [f"- {x}" for x in exceptions]
    naming = snapshot.get("naming_standard")
    if naming:
        lines += ["", "## Naming Standard", "", f"- Pattern: `{naming}`", "- Physical note/folder names are retained through the alias map; the canonical code is the registry identity."]
    legacy_rows = [(x["code"], x["legacy"]) for x in projects if x.get("legacy")]
    if legacy_rows:
        lines += ["", "## Legacy To Canonical Name Map", "", "| Canonical Code | Legacy / Alias Names |", "|---|---|"]
        lines += ["| " + code + " | " + "; ".join(names).replace("|", "\\|") + " |" for code, names in legacy_rows]
    discovery = snapshot.get("discovery")
    if discovery:
        lines += ["", "## Discovery / Vendor Assessment (Not Registered)", "",
        ...
        for d in discovery:
            lines.append("| " + " | ".join(str(v).replace("|", "\\|").replace("\n", " ") for v in (d.get("label"), d.get("vendor"), d.get("status"), d.get("scope"), "; ".join(d.get("open_questions", [])) or "-")) + " |")
    return "\n".join(lines).rstrip() + "\n"
