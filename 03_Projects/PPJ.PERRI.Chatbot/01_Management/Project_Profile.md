---
type: "project_profile"
project: "PPJ.PERRI.Chatbot.v3.2"
source_project: "PPJ.PERRI.Chatbot.md"
source_event: "Canonical naming populated"
last_verified: "2026-07-13"
confidence: "Strong"
documentation_status: "Current Working Document"
generated_by: "upgrade_ppj_project_workspaces.py"
---

<!-- PPJ_PORTFOLIO_SNAPSHOT_20260824_START -->
## Current State - 2026-09-18

| Field | Value |
| --- | --- |
| Canonical Code | AI_PERRIPlatform_v3.2.0 |
| Domain | Internal Chatbot & AI Platforms |
| Delivery Stream | INTERNAL DEVELOPMENT |
| Delivery Stage | GO-LIVE / PRODUCTION / SUPPORT |
| Lifecycle | Production |
| Status | Active |
| Progress | TBD |
| Gate | Permissioned production orchestration |
| Priority | Support |
| Outcome | Operate PPJ's conversational orchestration platform: User -> PERRI -> Intent -> Agent / Tool -> Data / Knowledge / API -> Controlled Response / Action. |
| Source Event | PPJ-PORTFOLIO-SNAPSHOT-20260918 |

### Latest Update

Production. PERRI is an orchestration platform and should not absorb every business-domain project. Read-only agents and action agents remain distinct; action agents require stronger permissions, approval and audit.

### Current Risks

- Action-agent permissions and audit

### Dependencies

- None recorded in the current portfolio snapshot.

### Next Actions

- Maintain permission and approval controls
- Monitor tool execution and audit logs
<!-- PPJ_PORTFOLIO_SNAPSHOT_20260824_END -->

# Project Profile

## Identity

| Field | Value |
| --- | --- |
| Canonical Code | PPJ.PERRI.Chatbot.v3.2 |
| Physical Project Note | PPJ.PERRI.Chatbot.md |
| Primary Domain | Internal Chatbot & AI Platforms |
| Cluster | AI Orchestrator / Internal Chatbot |
| Lifecycle Class | C. Production / Maintenance |
| Lifecycle | Production / Support |
| Current Gate | Permission Enhancement |
| Priority | TBD |
| Business Owner | Needs Confirmation |

## One-Line Understanding

Internal chatbot/orchestrator connecting users to knowledge base, APIs, tools, agents, and workflows.

## Intended Outcome

Production chatbot with department-level agents, permissions, logging, and controlled tool/API calling.

## Users and Delivery Participants

- Primary users: PPJ departments / Khoa / Nam
- BA / Coordination: Needs Confirmation
- Technical members: Nam

## Current Scope

- AI Orchestrator layer for PERRI, department agents, knowledge lookup, and controlled automation triggers.

## Explicit Boundaries

- Not a simple FAQ bot and not unrestricted access to all departments/tools.

## Current Gate and Next Move

- Gate: Permission Enhancement
- Next actions: Map PERRI access roles by department.; Define approved tool catalog with PPJ.AI.Hub.v2.1.; Confirm Invoice Downloader integration scope.; Draft fallback and escalation rules.; Define audit log schema.

## Evidence Basis

- Root project note: [[../PPJ.PERRI.Chatbot]]
- Project memory: [[03_Projects/_Registry/Project_Memory/PPJ.PERRI.Chatbot.v3.2.memory]]
- Source event: Canonical naming populated
- Last verified: 2026-07-13
- Confidence: Strong
